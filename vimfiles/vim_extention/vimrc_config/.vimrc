...
if     has('win32')             
    ...
    let g:cr_slash = '/' | let g:cfg_path_main = $HOME | let g:cfg_path = cfg_path_main . g:cr_slash . '.vim' . g:cr_slash . 'vimfiles'  | set runtimepath+=~/.vim/vimfiles "path were to place plugins in ubuntu
endif | endif
...
map \ds  :call FWK_Diff_SameBuffer()<CR>
...
if has("autocmd")
    ...
    autocmd FileType python,java inoremap <buffer> <expr> . FWK_System_Dot_MayComplete()
    ...
    "autocmd FileType python,java,html,php,c,cpp,cs,ocaml call FWK_pasteDoubleQuote_init_map("'", "'")
    autocmd  FileType python,java,html,php,c,cpp,cs,ocaml,tex call FWK_pasteDoubleQuote_init_map('(', ")")
...
"function! TextEnableCodeSnip(filetype,start,end,textSnipHl) abort
  ...
  "\ contains=@'.group
"endfunction
"autocmd FileType conf call TextEnableCodeSnip('cpp', '@code_start=cpp@', '@code_end=cpp@', 'SpecialComment')
"HIGHLIGHT
autocmd FileType conf syntax include @CPP   syntax/cpp.vim
autocmd FileType conf syntax include @JAVA  syntax/html.vim
"autocmd FileType conf syntax include @PYHON syntax/python.vim
"autocmd FileType conf syntax include @XML   syntax/xml.vim
""autocmd FileType conf syntax include @PYTHON syntax/python.vim

autocmd FileType conf syntax region cppSnip   matchgroup=SSnip  start=".*@code_start=cpp@.*"   end=".*@code_end=cpp@.*"   contains=@CPP
"autocmd FileType conf syntax region javaSnip  matchgroup=Snip  start=".*@code_start=java@.*"  end=".*@code_end=java@.*"  contains=@JAVA
"autocmd FileType conf syntax region pySnip    matchgroup=Snip  start=".*@code_start=py@.*"    end=".*@code_end=py@.*"    contains=@PYHON
"autocmd FileType conf syntax region xmlSnip   matchgroup=XSnip start=".*@code_start=xml@.*"   end=".*@code_end=xml@.*"   contains=@XML
...
"map \gr :call MMake_GetTemplate(PLFM_VIM_TEMPLATES . cr_slash . 'config.vrun', 'config.vrun')<CR>
...
map <A-v> ciw<s-ins><Esc>
imap <A-v> <c-o>ciw<s-ins>
...
"run cmd
map  <A-\>      :!start cmd<cr>
vmap <A-\> <esc>:!start cmd<cr>a
imap <A-\> <esc>:!start cmd<cr>a
...
map <C-t>  :tabnew<CR>:tabmove999<CR>
imap <C-t>  <Esc>:tabnew<CR>:tabmove999<CR>
"vert split 
...
let g:FWK_Grep_exclude_files = 'tags *.svg *.hbac *.hbsm *.hbcm *.xml *.uml *.valid *.test *.uxf'
...
map \gc : lcd %:p:h<CR>:LGrep -riI "" *<C-Left><C-Left><Right>
