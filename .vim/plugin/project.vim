...
if !exists('g:proj_window_number')  
  let g:proj_window_number=0
endif 
...
function! s:Project(filename) " <<<
    ...
    if !exists("g:proj_running") || (bufwinnr(g:proj_running) == -1) " Open the Project Window
        ...
        setlocal nomodeline
        "window number ADDED"
        let g:proj_window_number = winnr()
    else
        silent! 99wincmd h
...
" manual function 
    ...
    function! CreateEntriesFromDir(recursive, arg_name, arg_dir, arg_c_d, arg_filter_directive)
        ...
        let filter_directive = a:arg_filter_directive
...
" ---------------------------------------
    ...
    function! s:CreateEntriesFromDir(recursive)
        ...
        if (match(g:proj_flags, '\Cb') != -1) && has('browse')
            ...
            " manual edit: remove gui browse (buggy in ubuntu)
            let dir = inputdialog('Enter path to Entry: ')
            "let dir = browse(0, 'Enter the '.absolute.'Directory to Load: ', '', '')
            let dir = fnamemodify(dir, ':p')
        else
            let dir = inputdialog('Enter the '.absolute.'Directory to Load: ', '')
    ...
    function! s:RefreshEntriesFromDir(recursive)
        ...
        "let isFoldClosed = foldclosed(line("."))
        " Open the fold.
        if getline('.') =~ '}'
            normal! zo[z
        else
    ...
    if !exists("g:proj_running")
        ...
        "nmap     <buffer> <silent> <LocalLeader>sh <S-Return>
        ...
        "CHANGED
        nmap     <buffer> <silent> <LocalLeader>v <CR>`<Left>
        nmap     <buffer> <silent> <LocalLeader>q <CR>`<Left>
        ...
        nnoremap <buffer> <silent>           <F4> :call FWK_Project_Addon_refresh_path()<CR>
