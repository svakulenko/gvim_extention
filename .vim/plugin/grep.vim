...
function! RunGrep(cmd_name, grep_cmd, action, ...)
    if a:0 > 0 && (a:1 == "-?" || a:1 == "-h")
        echo 'Usage: ' . a:cmd_name . " [<grep_options>] [<search_pattern> " .
                        \ "[<file_name(s)>]]"
        return
...
if !exists('g:FWK_Grep_exclude_files')
    let g:FWK_Grep_exclude_files        = ''
endif
if !exists('g:FWK_Grep_exclude_dir')
    let g:FWK_Grep_exclude_dir        = ''
endif


"Description: addon to grep to make in win32 work grep 'exclude' and 'exclude-dir' which normally not work"
"This function genarete exclude files/dirs string in format convenable for grep
func! FWK_Grep_getExcludeString()
    if !empty( g:FWK_Grep_exclude_dir )
        let str = ''
        for dir_ext in split(g:FWK_Grep_exclude_dir,'\s')
            let str .= '--exclude-dir='     . dir_ext . ' '
        endfor
    endif

    if !empty( g:FWK_Grep_exclude_files )
        for file_ext in split(g:FWK_Grep_exclude_files,'\s')
            let str .= '--exclude='     . file_ext . ' '
        endfor
    endif
...
"Description: addon to grep to make in win32 work grep 'exclude' and 'exclude-dir' which normally not work"
"This function is resends parameters in form costum exclude string + <f-args>
func! FWK_Grep_Alias(...)

   "call s:RunGrep('Grep', 'grep', 'set', a:000)
    let grep_str  = 'call RunGrep(' . "'" . 'Grep'. "'" . ',' . "'" . 'grep' . "'" . ',' . "'" .'set' . "'"

    for arg_ in split(FWK_Grep_getExcludeString(),'\s')
            let grep_str .= ','
            let grep_str .= "'" . arg_ . "'"
    endfor
    for arg_ in a:000
...
command! -nargs=* -complete=file LGrep call FWK_Grep_Alias(<f-args>)

"Description: addon to grep to make in win32 work grep 'exclude' and 'exclude-dir' which normally not work"
"Here i only remove s: indentificator from function RunGrep

" Define the set of grep commands
command! -nargs=* -complete=file Grep
            \ call RunGrep('Grep', 'grep', 'set', <f-args>)
...
command! -nargs=* -complete=file FgrepAdd
            \ call RunGrep('FgrepAdd', 'fgrep', 'add', <f-args>)
...
command! -nargs=* -complete=file EgrepAdd
            \ call RunGrep('EgrepAdd', 'egrep', 'add', <f-args>)
...
command! -nargs=* -complete=file AgrepAdd
            \ call RunGrep('AgrepAdd', 'agrep', 'add', <f-args>)
