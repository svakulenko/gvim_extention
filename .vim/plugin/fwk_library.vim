...
func FWK_Extention_Switch(mode)
    let dict_ext_key = expand ("%:e") "get ext
    ...
    if dict_ext_key != ''
        ...
        if has_key(g:FWK_file_ext_dictionary, dict_ext_key) "have special extentions ? let's replace by them
            ...
            elseif a:mode == 'project_switch' "switch to file in a 'Project'
                ...
                let l:prj_path = substitute(expand ("%:p:h"),'\/','\\\/','ge')
                ...
                call search(l:prj_path) "search for prj path

                let search_result = 0
                for ext in extList
                    let search_result = search('\<' . file_name . '.' . ext  . '\>')
                    if ( search_result > 0 ) | break | endif
                endfor

                if search_result == 0
                    call search('\<' . expand("%:t")  . '\>') "focus this file
                endif
                ...
                if search_result == 0
                    echo 'FWK_Extention_Switch: cant find file:"' . file_name . '", with possible extentions:' . join(extList,",") . '. Skip action ...'
                    return
                endif
...
func FWK_System_Dot_MayComplete(mode)
    ...
    if pre_last_char =~ '[a-zA-Z0-9(]'
        ...
        if     a:mode == 0 " pop_first
            return ".\<C-X>\<C-O>\<C-N>" 
        elseif a:mode == 1 " pop_only; not work properly
            return ".\<C-X>\<C-O>" 
        endif
...
func FWK_pasteDoubleQuote(t_symbol_open, t_symbol_close)
    ...
    "past symbols
    "Decho("lbrackets:" . lbrackets)
    "Decho("rbrackets:" . rbrackets)
     "let lbrackets = len(split(getline("."),'[^(]*'))
     "let rbrackets = len(split(getline("."),'[^)]*'))
     if lbrackets == rbrackets
    exe "normal " . l_insertmode . a:t_symbol_open . a:t_symbol_close
     else if lbrackets > rbrackets
         exe "normal " . l_insertmode . a:t_symbol_close
     else
         exe "normal " . l_insertmode . a:t_symbol_open
     endif
...
"--------------------------------------------------------------------
"Description: trigger COPEN/CCL command (quick fix window operations, open if closed, close  
"if opened)  change path to current dir
"--------------------------------------------------------------------
...
"
"Description: diff current file with svn base
"
function! Fwk_diff_svn_current_file()

    call FWK_save_curr_path()
    call FWK_change_curr_path()

    exe '!svn diff ' . expand ("%:t")
