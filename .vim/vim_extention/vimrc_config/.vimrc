...
"FWK_CTAGS PLUGIN
runtime plugins/fwk_ctags.vim  "plugin
"runtime fwk_ctags_config.vim   "configuration file
...
"if PLFM_HOSTNAME == 'dell-craventos'
    ...
    exe  "map ²c :OpenWindowSilent " . PLFM_VIMCFG . cr_slash . "fwk_ctags_config.vim" . "<CR>"
"else
    exe  "map `o :OpenWindowSilent " . PLFM_VIMCFG . cr_slash . ".vimrc" . "<CR>"
    exe  "map `c :OpenWindowSilent " . PLFM_VIMCFG . cr_slash . "fwk_ctags_config.vim" . "<CR>"
"endif
...
map <F5> :exec("tag /".expand("<cword>"))<CR>
map <F6> :exec("tselect /".expand("<cword>"))<CR>
...
autocmd FileType conf,notes call TextEnableCodeSnip('perl'       , '@begin=perl@'   , '@end=perl@'      , 'SpecialComment')
...
autocmd FileType conf,notes call TextEnableCodeSnip('tex'        , '@begin=tex@'    , '@end=tex@'       , 'SpecialComment')
...
let g:fwk_paste_indentexpr= {}
func SafePaste()
    let g:fwk_paste_indentexpr = {'inde' : &inde, 'ai' : &ai, 'cindent' : &cindent }
    set inde=
    set noai
    set nocindent
endfunc
func SafePasteRestore()
    "echo 'indent:' . a:val['inde']
    exe 'set inde=' .  g:fwk_paste_indentexpr['inde']
    set autoindent
    set cindent
    "exe "normal " . "\<C-R>+"
endfunc
...
if has("gui_running")
    ...
    "imap <silent> <S-Insert> <C-R>+
    "imap <silent> <S-Insert> <C-O>:call SafePaste()<CR><C-R>+
    "imap <CR> <CR> <BS>
    
    "super tip to not lose intent on new line, if we press <Esc> or <C-O> !!
    "Minus: - that make vim slower
    "http://vim.wikia.com/wiki/Prevent_autoindent_from_removing_indentation
    inoremap <CR> <CR><Space><BS>
    nnoremap o o<Space><BS>
    nnoremap O O<Space><BS>
    
    imap <silent> <S-Insert> <C-O>:call SafePaste()<CR><C-R>+<C-O>:call SafePasteRestore()<CR>
    "imap <silent> <S-Insert> <C-R>+
    "imap <silent> <S-Insert> <C-O>:echo "check console mode"<CR><C-R>+
    "imap <silent> <S-Insert> <C-\><C-O>:echo "check console mode"<CR>
    "imap <silent> <S-Insert> <C-R>+

    "imap <silent> <S-Insert> <C-O>:call SafePaste()<CR>
...
"runtime plugins/fwk_ctags.vim
let &tags  = g:PLFM_VIMCFG . g:PLFM_SL . 'tags'
let &tags .= ',' . g:PLFM_VIM_VIMFILES . g:PLFM_SL . 'tags'
""let &tags .= ',..\tags,tags'

if has('win32') "win32 home machine
    let g:fwk_ctags_config_to_load = 'D:\Dropbox\private\share_files\.vim\plugin\ctags_config_home_win32.cfg'
elseif has('mac') "my home mac
    echo 'please set g:fwk_ctags_config_to_load for mac ...' 
else "linux
    if hostname() == 'blackbox' "linux, home machine
        let g:fwk_ctags_config_to_load = '/home/sergey/Dropbox/private/share_files/.vim/plugin/ctags_config_home_linux.cfg'
    else "linux, work machine
        let g:fwk_ctags_config_to_load = '/home/svakulenko/Dropbox/private/share_files/.vim/plugin/ctags_config_work.cfg'
    endif
endif
